...
public abstract class MessageRecord extends DisplayRecord {
  ...
  private @NonNull String getGv2Description(@NonNull Context context) {
    try {
      ...
      if (decryptedGroupV2Context.hasChange() && decryptedGroupV2Context.getGroupState().getVersion() > 0) {
        ...
        return formatGv2Change(context, change, descriptionStrategy);
      } else {
        return formatNewGv2Group(context, decryptedGroupV2Context.getGroupState(), descriptionStrategy);
      }
    } catch (IOException e) {
   ...
   */
  private @NonNull String formatNewGv2Group(@NonNull Context context,
                                            @NonNull DecryptedGroup group,
                                            @NonNull ShortStringDescriptionStrategy descriptionStrategy)
  {
      ...
      if (foundingMember.isPresent()) {
        if (selfByteString.equals(foundingMember.get().getUuid())) {
          ...
          if (DecryptedGroupUtil.findMemberByUuid(group.getMembersList(), self).isPresent()) {
            return context.getString(R.string.MessageRecord_s_added_you, descriptionStrategy.describe(UuidUtil.fromByteStringOrUnknown(foundingMember.get().getUuid())));
          } else {
            ...
            if (selfPending.isPresent()) {
              return context.getString(R.string.MessageRecord_s_invited_you_to_the_group, descriptionStrategy.describe(UuidUtil.fromByteStringOrUnknown(selfPending.get().getAddedByUuid())));
            } else {
 ...
 private static @NonNull String formatGv2Change(@NonNull Context context,
                                                @NonNull DecryptedGroupChange change,
                                                @NonNull ShortStringDescriptionStrategy descriptionStrategy)
 {
   List<String>  strings = new GroupsV2UpdateMessageProducer(context, descriptionStrategy, Recipient.self().getUuid().get()).describeChange(change);
