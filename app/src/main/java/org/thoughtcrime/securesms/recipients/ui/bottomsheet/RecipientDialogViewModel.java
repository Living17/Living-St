...
final class RecipientDialogViewModel extends ViewModel {
  ...
  private final MutableLiveData<Boolean>                         recipientIsAdmin;
                                   ...
                                   @NonNull RecipientDialogRepository recipientDialogRepository)
  {
    ...
    this.adminActionBusy           = new DefaultValueLiveData<>(false);
    this.recipientIsAdmin          = new DefaultValueLiveData<>(false);

    RecipientId              selfId          = Recipient.self().getId();
    boolean                  recipientIsSelf = recipientDialogRepository.getRecipientId().equals(selfId);
    ...
    if (recipientDialogRepository.getGroupId() != null && recipientDialogRepository.getGroupId().isV2()) {
      recipientDialogRepository.isAdminOfGroup(selfId, localIsAdmin::setValue);
  ...
  public MutableLiveData<Boolean> getAdminActionBusy() {
                                      ...
                                      (dialog, which) -> {
                                        ...
                                        recipientDialogRepository.setMemberAdmin(true, result -> {
                                          ...
                                          if (!result) {
                                            ...
                                            recipientIsAdmin.postValue(true);
                                          }
                                        });
                                      })
                                      ...
                                      (dialog, which) -> {
                                        ...
                                        recipientDialogRepository.setMemberAdmin(false, result -> {
                                          ...
                                          if (!result) {
                                            ...
                                            recipientIsAdmin.postValue(false);
                                          }
                                        });
                                      })
                   .setNegativeButton(android.R.string.cancel, (dialog, which) -> {})
