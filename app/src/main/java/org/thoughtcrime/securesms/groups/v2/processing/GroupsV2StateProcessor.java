...
public final class GroupsV2StateProcessor {
  ...
  public final class StateProcessorForGroup {
    ...
    private final NetworkGroupStateProvider networkGroupStateProvider;
    ...
    private StateProcessorForGroup(@NonNull GroupMasterKey groupMasterKey) {
      ...
      this.networkGroupStateProvider = new NetworkGroupStateProvider(ApplicationDependencies.getSignalServiceAccountManager().getGroupsV2Api(),
        ...
        throws IOException, GroupNotAMemberException
    {
      ...
      if (!groupDatabase.isUnknownGroup(groupId) && revision != LATEST) {
        ...
        if (revision <= dbRevision) {
          return new GroupUpdateResult(revision == dbRevision ? GroupState.GROUP_CONSISTENT : GroupState.GROUP_AHEAD, null);
        }
      }
      ...
      if (newLocalState == null || newLocalState == inputGroupState.getLocalState()) {
        return new GroupUpdateResult(GroupState.GROUP_CONSISTENT, null);
      } else {
