  ...
  void acceptMessageRequest(@NonNull LiveRecipient liveRecipient, long threadId, @NonNull Runnable onMessageRequestAccepted) {
    executor.execute(()-> {
      ...
      if (liveRecipient.get().isPushV2Group()) {
        ...
        } catch (IOException | GroupChangeBusyException | GroupChangeFailedException | GroupNotAMemberException | GroupInsufficientRightsException e) {
          Log.w(TAG, e);
          // TODO: GV2 toast or something, make sure user can retry
        }
      } else {
